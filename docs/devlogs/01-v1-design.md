...
I designed with FDM 3D printing's strengths and weaknesses specifically in mind: layer orientation, load direction relative to layer lines, wall thickness, rather than just printing "machined part" geometry and hoping it holds up. At the same time, I kept track of which components are candidates to eventually remake in aluminum if the design is proven out, so the plastic-specific DFM choices don't paint me into a corner for a metal version down the line.
